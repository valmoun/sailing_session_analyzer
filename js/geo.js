export const MIN_DIST_M    = 1.5;
const EARTH_R_M     = 6_371_000;
export const M_S_TO_KTS    = 1.94384;
export const M_PER_NM      = 1852;
export const MAX_SPEED_KTS = 65;

// Distance calculation between two lat/lon points, in meters, using the Haversine formula.
...
export function compassBearing(p1, p2) {
  const r = d => d*Math.PI/180;
...
export function pointOfSail(twa, upMax, dwnMin) {
  if (twa < upMax)  return 'upwind';
  if (twa < dwnMin) return 'reach';
  return 'downwind';
}

export function vmgUpwind(spd, twa)   { return spd * Math.cos(twa*Math.PI/180); }
export function vmgDownwind(spd, twa) { return spd * Math.cos((180-twa)*Math.PI/180); }

export function movingAvg(arr, half) {
  return arr.map((_,i) => {
...
export function circularMovingAvg(bearings, half) {
  const r = d => d*Math.PI/180;
...
*/
export function bestWindowAvgKts(pts, windowSec) {
